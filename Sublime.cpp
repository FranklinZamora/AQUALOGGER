...
void setup()
{
    ...
    if (EEPROM.read(0) == 0x00 && EEPROM.read(1) == 0x00 && EEPROM.read(2) == 0x00 && EEPROM.read(3) == 0x00 && EEPROM.read(4) == 0x00 && EEPROM.read(5) == 0x00 && EEPROM.read(6) == 0x00 && EEPROM.read(7) == 0x00)
    {
        NodeDiscover();
    }
    else
    {
        Serial.println("[INFO]: Gateway found");
    }
}
...
void loop()
{
    ...
    // Change metod
    waterTemp.value = sht.getTemperature() * 100;
   ...
   while (Serial.available() > 0)
    ...
    int hourUTC = gps.time.hour() - 7;
    int minuteUTC = gps.time.minute();
    int secondsUTC = gps.time.second();
    int currentGPSday = gps.date.day();
